...
def plot_single_image(image, targets, path=None, fname="image.jpg", names=None):
    ...
    # prepare image for annotation
                # line width is scaled based on the smaller dimension of the image (either height or width), divided by 100.
                # Then rounds to the nearest whole number. By giving 2, regardless of the image size, it will at least have 2 pixels thick line width.
    annotator = Annotator(im_pil, line_width=max(round( min(im.shape[:2])/100 ), 2),
                # Scales the font size based on the smaller dimension either height or width then divided by 40. 
                # 12 ensures the text is readable, never less than 12. 
                            font_size=max(round(min(im.shape[:2]) / 40), 12), 
                            pil=True) # enables higher-quality text rendering  through PIL (Python Imaging Library).
    
    # Annotate
    if path:
        annotator.text([5, 5], text=Path(path).name[:40], txt_color=(220, 220, 220))  # filenames
    if len(targets) > 0:
        
        # Assuming targets format: [image_index, class_id, x_center, y_center, width, height, confidence]
        boxes = xywh2xyxy(targets[:, 2:6]).T
        #boxes = xywh2xyxy(targets[:, 2:6]).T # all rows, and columns from 2:6 which are [x_center, y_center, width, height]. 
        # In YOLO, column 0 is an identifier for the image and 1 is typically class index. 
        # (x_center, y_center, width, height) to (x_min, y_min, x_max, y_max). Easier to draw a box from top-left corner to bottom-right corner. 
        # .T to switches the rows and columns. Each bounding box can be accessed as a list of coordinates, which is convenient for further processing such as scaling and drawing.
        
        classes = targets[:, 1].astype("int") # targets[:, 1]: This retrieves the second column from the targets array, which contains the class indices of the detected objects. e.g. 'car'
        confidences = targets[:, 6]  # Confidence scores
        
        if boxes.shape[1]: # if boxes.shape[1] equals to 0, there are no detections. 
            if boxes.max() <= 1.01:  # boxes.max() <= 1.01 checks if the coordinates in boxes are normalized (typically between 0 and 1, with a small tolerance of 0.01
                boxes[[0, 2]] *= w  # If the coordinates are normalized, they are scaled to pixel dimensions. By * w, converting them from a relative scale to an absolute pixel scale
                boxes[[1, 3]] *= h
                for j, box in enumerate(boxes.T.tolist()):
                    cls = classes[j]
                    conf = confidences[j]
